    ...
    I2C: i2c::I2c<Error = E>,
{
        ...
        M: Inbox<ImuMessage>,
    {
        ...
        self.lsm6ds33.set_accelerometer_scale(AccelerometerScale::G02).await?;
        self.lsm6ds33.set_gyroscope_scale(GyroscopeFullScale::Dps245).await?;
        self.lsm6ds33.set_accelerometer_output(AccelerometerOutput::Rate104).await?;
        self.lsm6ds33.set_gyroscope_output(GyroscopeOutput::Rate104).await?;
        self.lsm6ds33.set_accelerometer_bandwidth(AccelerometerBandwidth::Freq400).await?;
        self.lsm6ds33.set_low_power_mode(true).await?;
        loop {
            let accel_raw = self.lsm6ds33.read_accelerometer().await?; // m/s^2
            let accel: [MeterPerSecond2<f32>; 3] = [accel_raw.0*MPS2, accel_raw.1*MPS2, accel_raw.2*MPS2];
            let gyro_raw = self.lsm6ds33.read_gyro().await?; // rads/s
            let gyro: [RadianPerSecond<f32>; 3] = [gyro_raw.0*RAD/S, gyro_raw.1*RAD/S, gyro_raw.2*RAD/S];
